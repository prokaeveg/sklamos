<?php session_start(); 
  ...
  $search = clear_string($_GET['q'], $connection);
...
<div class="row">
  ...
  <?php 
    ...
    if ($page != 1){ $pstr_prev = '<li><a class="pstr-prev" href="search.php?q='.$search.'&sort='.$sort.'&type='.$type.'&page='.($page - 1).'">&lt;</a></li>';}
    if ($page != $total) $pstr_next = '<li><a class="pstr-next" href="search.php?q='.$search.'&sort='.$sort.'&type='.$type.'&page='.($page + 1).'">&gt;</a></li>';

                                        
    // Формируем ссылки со страницами
    if($page - 5 > 0) $page5left = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&type='.$type.'&page='.($page - 5).'">'.($page - 5).'</a></li>';
    if($page - 4 > 0) $page4left = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&type='.$type.'&page='.($page - 4).'">'.($page - 4).'</a></li>';
    if($page - 3 > 0) $page3left = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&type='.$type.'&page='.($page - 3).'">'.($page - 3).'</a></li>';
    if($page - 2 > 0) $page2left = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&type='.$type.'&page='.($page - 2).'">'.($page - 2).'</a></li>';
    if($page - 1 > 0) $page1left = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&type='.$type.'&page='.($page - 1).'">'.($page - 1).'</a></li>';

    if($page + 5 <= $total) $page5right = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&type='.$type.'&page='.($page + 5).'">'.($page + 5).'</a></li>';
    if($page + 4 <= $total) $page4right = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&type='.$type.'&page='.($page + 4).'">'.($page + 4).'</a></li>';
    if($page + 3 <= $total) $page3right = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&type='.$type.'&page='.($page + 3).'">'.($page + 3).'</a></li>';
    if($page + 2 <= $total) $page2right = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&type='.$type.'&page='.($page + 2).'">'.($page + 2).'</a></li>';
    if($page + 1 <= $total) $page1right = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&type='.$type.'&page='.($page + 1).'">'.($page + 1).'</a></li>';

    if ($page+5 < $total)
    {
        $strtotal = '<li><p class="pagination-point">...</p></li><li><a href="search.php?q='.$search.'&sort='.$sort.'&type='.$type.'&page='.$total.'">'.$total.'</a></li>';
    }else
    {
        $strtotal = ""; 
    }
    ...
    <div class="pagination">
      <ul>
      ';
        echo $pstr_prev.$page5left.$page4left.$page3left.$page2left.$page1left."<li><a class='pagination-active' href='search.php?sort='.$sort.'&type='.$type.'page=".$page."'>".$page."</a></li>".$page1right.$page2right.$page3right.$page4right.$page5right.$strtotal.$pstr_next;
      echo '
      </ul>
    </div>
    ';
...
<?php } else { ?>
  <div class="col-md-9">
    <p>Поисковое значение должно быть от 2 до 64 символов!</p>
  </div>
<?php } ?>
  <div class="col-md-3">
